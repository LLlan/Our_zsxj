...
<body class="hold-transition skin-blue sidebar-mini">
	<div class="wrapper">
		...
		<header class="main-header"> <!-- Logo --> <a
			...
			<span class="logo-lg"><b>掌上巡检</b> </span> </a> <!-- Header Navbar --> <nav
			class="navbar navbar-static-top" role="navigation"> <!-- Sidebar toggle button-->
		<a href="#" class="sidebar-toggle" data-toggle="offcanvas"
			role="button"> <span class="sr-only">切换左侧按钮</span> </a> <!-- Navbar Right Menu -->
			...
			<ul class="nav navbar-nav">
							...
							<ul class="menu">
								<li>
									<!-- start notification --> <a href="#"> <i
										class="fa fa-users text-aqua"></i> 5 位同学新加入我们 </a>
								</li>
								<!-- end notification -->
							</ul>
						</li>
				...
				<li class="dropdown user user-menu">
					...
					data-toggle="dropdown"> <!-- The user image in the navbar--> <img
						...
						<span class="hidden-xs">James</span> </a>
					<ul class="dropdown-menu">
				...
				<div class="col-md-12">
					<div class="box box-primary">
						...
						<form role="form" id="form1"
							action="projectplan?action=update&pP_ID=${model.pp_ID }"
							method="post">
							<div class="box-body">

								<div class="form-group">
									<label for="pP_ProjectName">项目（工程）名称</label> <input type="text"
										class="form-control" id="pP_ProjectName" name="pP_ProjectName"
										placeholder="请输入项目（工程）名称" value="${model.pp_ProjectName }" />

								</div>
								<div class="form-group">
									<label for="pP_ProjectNo">项目编号</label> <input type="text"
										class="form-control" id="pP_ProjectNo" name="pP_ProjectNo"
										placeholder="请输入项目编号" value="${model.pp_ProjectNo }" />
								</div>
								<div class="form-group">
									<label for="pP_ExecComName">施工单位</label> <select
										id="pP_ExecComName" class="form-control" name="pP_ExecComName">
										<option selected="selected">请选择施工单位</option>
										<c:if test="${requestScope.branchs !=null }">
											<c:forEach items="${requestScope.branchs}" var="branch">
												<c:if
													test="${requestScope.model.pp_ExecComName == branch.branch_Name }">
													<option value="${branch.branch_ID}" selected="selected">${branch_Name}</option>
												</c:if>
												<c:if
													test="${requestScope.model.pp_ExecComName != branch.branch_Name }">
													<option value="${branch.branch_ID}">${branch_branch_ID}</option>
												</c:if>
											</c:forEach>
										</c:if>
									</select>
								...
								<div class="form-group">
									<label for="users_ID">随工人员</label> <select id="users_ID"
										class="form-control" name="users_ID">
										<option selected="selected">请选择随工人员名字</option>
									</select>

									<c:if test="${requestScope.users !=null }">
										<c:forEach items="${requestScope.users}" var="user">
											<c:if
												test="${requestScope.model.users_ID == user.users_ID }">
												<option value="${users.users_ID}" selected="selected">${users.users_TrueName}</option>
											</c:if>
											<c:if
												test="${requestScope.model.users_ID != user.users_ID }">
												<option value="${users.users_ID}">${users.users_TrueName}</option>
											</c:if>
										</c:forEach>
									</c:if>
								</div>
								<div class="form-group">
									<label for="pP_Content">设计内容</label> <input type="text"
										class="form-control" id="pP_Content" name="pP_Content"
										placeholder="请输入设计内容" value="${model.pp_Content }" />
								</div>
								<div class="form-group">
									<label for="pP_BOMList">物料清单</label> <input type="text"
										class="form-control" id="pP_BOMList" name="pP_BOMList"
										placeholder="请输入物料清单" value="${model.pp_BOMList }" />
								</div>
								<div class="form-group">
									<label for="pP_StartDate">开工日期</label> <input type="text"
										class="form-control" id="pP_StartDate" name="pP_StartDate"
										placeholder="请输入开工日期" value="${model.pp_StartDate }" />
								</div>
								<div class="form-group">
									<label for="pP_EndDate">完工日期</label> <input type="text"
										class="form-control" id="pP_EndDate" name="pP_EndDate"
										placeholder="请输入完工日期" value="${model.pp_EndDate }" />
								</div>
		...
		<!-- Default to the left --> <strong>Copyright &copy; 2015 <a
			href="#">掌上巡检</a>. </strong> All rights reserved. </footer>
										...
										.post(
												...
												function(data) {
													...
													var json = $
															.parseJSON(data);
													//遍历json数组
													var str = "<option selected='selected'>请选择随工人员名字</option>";
													$
															.each(
																	json,
																	function(i,
																			value) {
																		//初始化选项
																		str += "<option value = '"+value.users_ID+"'>"
																				+ value.users_Name
																				+ "</option>";
																	});
