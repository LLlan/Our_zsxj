					...
					<div class="col-md-12"> 
						<div class="box box-primary"> 
							...
							<form role="form" id = "form1"
								action="menus?action=update&menus_ID=${model.menus_ID}" 
								method="post"> 
								<div class="box-body"> 
									...
									<div class="form-group"> 
										<label for="menus_Title">菜单标题</label> <input type="text" 
											...
											placeholder="menus_Title" value="${model.menus_Title}"> 
									</div> 
									<div class="form-group"> 
										<label for="menus_Url">菜单链接</label> <input type="text" 
											class="form-control" id="menus_Url" name="menus_Url" 
											placeholder="menus_Url" value="${model.menus_Url}"> 
									</div> 
									<div class="form-group"> 
										<label for="menus_PID">父级菜单</label> 
										<select name="menus_PID">
                                            	<%-- <option value = "0">顶级</option>
                                            	<c:if test="${requestScope.menus !=null }">
                                            		<c:forEach items="${requestScope.menus}" var="menu">
                                            			
                                            			<c:if test="${requestScope.model.menus_PID != menu.menus_ID }">
                                            				<option value = "${menu.menus_ID }">${menu.menus_Title }</option>
                                            			</c:if>
                                            			
                                            			<c:if test="${requestScope.model.menus_PID == menu.menus_ID }">
                                            				<option value = "${menu.menus_ID }" selected="selected">${menu.menus_Title }</option>
                                            			</c:if>
                                            			
                                            		</c:forEach>
                                            	</c:if> --%>
                                            	<c:out value="${requestScope.menuTree }" escapeXml="false"></c:out>
                                         </select>
									</div> 
